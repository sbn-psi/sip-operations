...
def main():
    ...
    delta_sip = sipdelta.generate_delta(old_sips, sip, dest)

    old_transfers = find_files(args.old_dir, ".*transfer.*tab")
    transfer = find_files(args.new_dir, ".*transfer.*tab")[0]
    delta_transfer = transferdelta.generate_transfer_delta(transfer, old_transfers, dest)

    old_checksums = find_files(args.old_dir, ".*checksum.*tab")
    checksum = find_files(args.new_dir, ".*checksum.*tab")[0]
    delta_checksum = checksumdelta.generate_checksum_delta(checksum, old_checksums, dest)

    aip_label = find_files(args.new_dir, ".*aip.*xml")[0]
    generated_aip_label = aiplabel.gen_aip_label(delta_checksum, delta_transfer, aip_label, dest)

    sip_label = find_files(args.new_dir, ".*sip.*xml")[0]
    siplabel.gen_sip_label(delta_sip, sip_label, generated_aip_label, dest)
